...
where proname in ('is_workspace_member','workspace_role','create_workspace_with_owner','prevent_last_owner_demotion','get_my_workspaces');
```
Expected 5 rows: `is_workspace_member` (s/false), `workspace_role` (s/false), `create_workspace_with_owner` (v/true), `prevent_last_owner_demotion` (v/false), `get_my_workspaces` (s/false).
