...
| Delete with "move target" — UPDATE projects fails partway | Wrap in a Supabase RPC for atomicity (recommended), OR rely on RESTRICT to block the workspace delete and surface a specific toast naming the failing UPDATE. RESTRICT acts as the "you forgot to handle some projects" canary. |
